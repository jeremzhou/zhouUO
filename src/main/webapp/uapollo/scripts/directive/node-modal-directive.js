/*directive_module.directive('nodemodal', nodeModalDirective);

function nodeModalDirective(toastr, $sce, AppUtil, EventManager, ConfigService, NodeService) {
    return {
    	restrict: 'E',
        templateUrl: '../../views/component/node-modal.html',
        transclude: true,
        replace: true,
        link: function (scope) {*/
          ...
          /*  var TABLE_VIEW_OPER_TYPE = {
           ...
           /* scope.createNode = createNode;
            ...
            /*function doNode(){
            	...
            	if(scope.tableViewType == TABLE_VIEW_OPEN_TYPE.CREATE){
            		if(scope.toOperationNamespace.isBranch){
                		NodeService.create_node(scope.selectproject,
                								scope.nodeName).then(
                					function (result){
                						toastr.success("添加成功！");
                						AppUtil.hideModal('#nodeModal');
                					},function (result){
            		...
            		NodeService.update_node(scope.selectproject,
            								scope.nodeName).then(
            							function (result){
            								toastr.success("修改成功！");
            								AppUtil.hideModal("#nodeModal");
